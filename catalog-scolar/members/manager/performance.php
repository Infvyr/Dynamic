<?php 
session_start(); 
$title = "Performanța copilului dvs.";
...
// total marks
$query1 = "SELECT note.user_name, note.user_class, COUNT(note.disciplina) FROM note INNER JOIN users ON users.user_name = '$Q' AND users.user_child = note.user_name";
$res = mysqli_query($db,$query1);
$row1 = mysqli_fetch_row($res);

// total abs
$query2 = "SELECT abs.user_name, abs.user_class, COUNT(abs.disciplina) FROM abs INNER JOIN users ON users.user_name = '$Q' AND users.user_child = abs.user_name";
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="box">
                    ...
                    <div class="box-header with-border bg-green">
                        <h3 class="box-title">Randamentul copilului dvs. - <?= '<strong>' .$result[1].'</strong>'; ?></h3>
                    </div>
                    <div class="box-body">
                        <div class="row">
                            <div class="box-body">
<?php
    ...
    if(isset($_POST["show"])) {
        ...
        $sql = "SELECT note.disciplina, note.user_name, ROUND(AVG(note.nota), 3), COUNT(note.nota), note.user_class FROM note INNER JOIN users ON users.user_child = note.user_name AND users.user_name = '$Q' AND note.disciplina = '$sel_disc'";
    ...
    $chartQuery = "SELECT
                    ...
                    FROM note INNER JOIN
                        users ON users.user_child = note.user_name AND users.user_name = '$Q'
    ...
    // abs chart
    $absQuery = "SELECT abs.disciplina, abs.user_name, COUNT(*) AS NumberOfAbs, abs.user_class FROM abs INNER JOIN users ON users.user_child = abs.user_name AND users.user_name = '$Q' GROUP BY abs.disciplina";
